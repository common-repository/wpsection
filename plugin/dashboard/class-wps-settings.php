...
if ( ! class_exists( 'PB_Settings' ) ) {
	class PB_Settings {
		...
		function required_plugin_check() {
			...
			if ( count( $buttons ) > 0 ) printf( '<div class="notice notice-error is-dismissible"><p>%1$s</p><p>%2$s</p></div>',

												
sprintf(
    // translators: %1$s: Plugin name, %2$s: List of required plugins.
    esc_html__( '<strong>%1$s</strong> plugin requires plugin(s): %2$s to be installed and activated. Please continue with installation or activation', 'wpsection' ),
    esc_html( $this->get_data( 'plugin_name' ) ),
    esc_html( implode( ', ', $plugins ) )
),
esc_html( implode( ' ', $buttons ) )
...
function register_taxonomy( $tax_name, $obj_type, $args = array() ) {
    ...
    $args = array_merge( array(
        // translators: %s: The plural form of the taxonomy being managed.
        'description'         => sprintf( __( 'This is where you can create and manage %s.', 'wpsection' ), $plural ),
        'public'              => true,
        'show_ui'             => true,
    ...
    $args['labels'] = array_merge( array(
        // translators: %s: The plural form of the taxonomy name.
        'name'               => sprintf( __( 'This %s', 'wpsection' ), $plural ),
        'singular_name'      => $singular,
        'menu_name'          => esc_html( $plural ),
        // translators: %s: The plural form of the taxonomy name.
        'all_items'          => sprintf( __( 'All %s', 'wpsection' ), $plural ),
        // translators: %s: The singular form of the taxonomy name.
        'add_new'            => sprintf( __( 'Add %s', 'wpsection' ), $singular ),
        // translators: %s: The singular form of the taxonomy name.
        'add_new_item'       => sprintf( __( 'Add %s', 'wpsection' ), $singular ),
        'edit'               => __( 'Edit', 'wpsection' ),
        // translators: %s: The singular form of the taxonomy name.
        'edit_item'          => sprintf( __( 'Edit %s', 'wpsection' ), $singular ),
        // translators: %s: The singular form of the taxonomy name.
        'new_item'           => sprintf( __( 'New %s', 'wpsection' ), $singular ),
        // translators: %s: The singular form of the taxonomy name.
        'view'               => sprintf( __( 'View %s', 'wpsection' ), $singular ),
        // translators: %s: The plural form of the taxonomy name.
        'search_items'       => sprintf( __( 'Search %s', 'wpsection' ), $plural ),
        // translators: %s: The plural form of the taxonomy name.
        'not_found'          => sprintf( __( 'No %s found', 'wpsection' ), $plural ),
        // translators: %s: The plural form of the taxonomy name.
        'not_found_in_trash' => sprintf( __( 'No %s found in trash', 'wpsection' ), $plural ),
        // translators: %s: The singular form of the taxonomy name.
        'parent'             => sprintf( __( 'Parent %s', 'wpsection' ), $singular ),
    ), $labels );
...
function register_post_type( $post_type, $args = array() ) {
    ...
    $args = array_merge( array(
        // translators: %s: The plural form of the post type being managed.
        'description'         => sprintf( __( 'This is where you can create and manage %s.', 'wpsection' ), $plural ),
        'public'              => true,
        'show_ui'             => true,
        'capability_type'     => 'post',
    ...
    $args['labels'] = array_merge( array(
        // translators: %s: The plural form of the post type name.
        'name'               => sprintf( __( 'This %s', 'wpsection' ), $plural ),
        'singular_name'      => $singular,
        // translators: %s: The plural form of the post type name.
        'menu_name'          => sprintf( __( 'This %s', 'wpsection' ), $plural ),
        // translators: %s: The plural form of the post type name.
        'all_items'          => sprintf( __( 'This %s', 'wpsection' ), $plural ),
        // translators: %s: The singular form of the post type name.
        'add_new'            => sprintf( __( 'Add %s', 'wpsection' ), $singular ),
        // translators: %s: The singular form of the post type name.
        'add_new_item'       => sprintf( __( 'Add %s', 'wpsection' ), $singular ),
        'edit'               => __( 'Edit', 'wpsection' ),
    ...
    printf( '<div>%s</div>', esc_html( $setting_section['title'] ) );
}
if ( isset( $setting_section['description'] ) ) {
    printf( '<p>%s</p>', esc_html( $setting_section['description'] ) );
}
    ...
    printf( '<span class="disabled-notice">%s</span>', esc_html( $this->get_disabled_notice() ) );
}
 ...
 <style>
    ...
    $field_options = wp_json_encode( $field_options ); // Use wp_json_encode instead of json_encode
    $field_options = preg_replace( '/"([^"]+)"\s*:\s*/', '$1:', $field_options );
    ...
    <select <?php echo esc_attr( $disabled ); ?> <?php echo esc_attr( $required ); ?> <?php echo esc_attr( $multiple_text ); ?>
        ...
        foreach ( $args as $key => $name ) {
            ...
            printf( '<option %s value="%s">%s</option>', esc_attr( $selected ), esc_attr( $key ), esc_html( $name ) );
        }
        ?>
    </select>
    <script>
...
function generate_timepicker( $option ) {
    ...
    wp_enqueue_script( 'jquery-ui-timepicker', WPSECTION_PLUGIN_URL . '/assets/jquery.timepicker.min.js', array( 'jquery' ), false, true );
    ...
    <?php
		...
		function generate_select( $option ) {
            ...
            <select <?php echo esc_attr( $disabled ); ?> <?php echo esc_attr( $required ); ?>
				...
				foreach ( $args as $key => $name ) {
					...
					printf( '<option %s value="%s">%s</option>', selected( $value, $key, false ), esc_attr( $key ), esc_html( $name ) );
		...
		function generate_checkbox( $option ) {
			...
			printf( '<fieldset>%s</fieldset>', implode( '<br>', array_map( 'esc_html', $cb_items ) ) );
...
foreach ( $args as $key => $val ) {
    ...
    printf(
        '<label><input %1$s %2$s type="radio" name="%3$s[]" value="%4$s">%5$s</label><br>',
        esc_attr( $disabled ),
        esc_attr( $checked ),
        esc_attr( $option_id ),
        esc_attr( $key ),
...
?>
    ...
    foreach ( $args as $key => $val ) {
        $checked = is_array( $value ) && in_array( $key, $value ) ? 'checked="checked"' : '';
        printf(
            '<label class="%2$s"><input %1$s %2$s type="%6$s" name="%3$s[]" value="%4$s"><img src="%5$s" /></label>',
            esc_attr( $disabled ),
            esc_attr( $checked ),
            esc_attr( $option_id ),
            esc_attr( $key ),
            esc_url( $val ), // Assuming $val contains image URLs, use esc_url for security
            esc_attr( $input_type ) // Ensure $input_type is properly sanitized
        );
    }
    ?>
</div>
    ...
    <div class="wrap">
        ...
        do_action( 'pb_settings_before_page_' . esc_attr( $this->get_current_page() ), $this );

        $this->get_settings_nav_tab();

        if ( $this->show_submit_button() ) {
            printf(
                '<form class="pb_settings_form" action="options.php" method="post">%s%s</form>',
                wp_kses_post( $this->get_settings_fields_html() ),
                wp_kses_post( get_submit_button() )
            );
        } else {
            echo wp_kses_post( $this->get_settings_fields_html() );
        }

        do_action( 'pb_settings_fields_area', $this );

        do_action( esc_attr( $this->get_current_page() ), $this );

        do_action( 'pb_settings_after_page_' . esc_attr( $this->get_current_page() ), $this );
        ?>
    </div>
    <?php
}
...
<nav class="nav-tab-wrapper">
    ...
    foreach ( $this->get_pages() as $page_id => $page ) {
        ...
        $nav_url_args = array( 'tab' => $page_id );
        $nav_menu_url = esc_url( add_query_arg( $nav_url_args, $pagenow ) );
        $page_nav     = isset( $page['page_nav'] ) ? esc_html( $page['page_nav'] ) : '';

        printf( '<a href="%s" class="nav-tab %s">%s</a>', esc_url( $nav_menu_url ), esc_attr( $active ), esc_html( $page_nav ) );
    }
...
function get_taxonomies_array( $string, $option ) {
    ...
    if ( ! taxonomy_exists( $taxonomy ) ) {
        return new WP_Error( 'not_found', sprintf( 'Taxonomy <strong>%s</strong> does not exist!', $taxonomy ) );
    }

    $terms = get_terms( array(
        'taxonomy'   => $taxonomy,
        'hide_empty' => false, // Deprecated, but still functional for backward compatibility
    ) );
...
function get_current_page() {
    ...
    if ( ! wp_verify_nonce( $nonce, 'your_nonce_action_here' ) ) {
        // Handle invalid nonce scenario here (e.g., exit, redirect, error handling)
        // Example: wp_die( 'Security check failed. Please try again.' );
