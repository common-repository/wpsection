...
function print_notice( $message = '', $is_success = true, $is_dismissible = true ) {
    ...
    // Sanitize $is_success to prevent injection.
    $is_success_class = is_bool( $is_success ) && $is_success ? 'success' : 'error';

    // Escape attributes before outputting.
    printf( '<div class="notice notice-%s %s"><p>%s</p></div>',
        esc_attr( $is_success_class ),
        esc_attr( $is_dismissible ? 'is-dismissible' : '' ),
        esc_html( $message )
    );
}
	...
	for ( $index = 1; $index <= $style_count; $index++ ) {
		// Translators: %s is the style number
