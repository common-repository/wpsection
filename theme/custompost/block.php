...
trait BlockPostType {
    ...
    public function register_post_type() {
        ...
        $labels = array(
            'name'                  => _x( 'Block Templates', 'Post Type General Name', 'wpsection' ),
            'singular_name'         => _x( 'Block Template', 'Post Type Singular Name', 'wpsection' ),
            'menu_name'             => __( 'Block Tmplt', 'wpsection' ),
...
class BlockMrMetaBoxes{
  ...
  <h4 style="font-size: 16px; font-weight: bold; margin-bottom: 10px;">Dinamic Shortcode</h4>
    ...
    <input type='text' class='widefat' value='[BLOCK_SHORTCODE id="<?php echo esc_attr( $post->ID ); ?>"]' readonly="">
    ...
    function column_data($column, $post_id){
        switch ( $column ) {
            ...
            case 'block_post_column' :
				...
				echo '<input type="text" class="widefat" value=\'[BLOCK_SHORTCODE id="' . esc_attr( $post_id ) . '"]\' readonly="">';
