...
<?php if (isset($settings['show_prduct_x_button']) && $settings['show_prduct_x_button']) : ?> 
    <?php if (!get_post_meta(get_the_ID(), 'meta_show_product_button', true)) : ?>                             
        <div class="wps_order order-<?php echo esc_attr($settings['position_order_eight']); ?> <?php echo esc_attr($unique_id); ?> hider_area_2">  

            <?php if (!empty($settings['wps_quick_view_button_link'])) : ?>       
            ...
            <?php else : ?> 
                <?php if ($product->is_in_stock()) : ?>
                    ...
                    <form class="cart wps_cart_qnt" action="<?php echo esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())); ?>" method="post" enctype='multipart/form-data'>
                        ...
                        <?php if (!empty($settings['wps_product_qun_hide'])) : ?>       
                        ...
                        <div class="cart-btn quick_defult_wps">
                            <button class="open-wps wps_button" type="submit" name="add-to-cart" value="<?php echo esc_attr($product->get_id()); ?>"> 
                                <?php if (!empty($settings['show_prduct_addtocart_icon'])) : ?>     
                         
							<i class="<?php echo esc_attr($settings['wps_product_adcart_icon']['value']); ?>"></i>
