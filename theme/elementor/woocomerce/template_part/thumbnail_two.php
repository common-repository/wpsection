...
<?php if ($thumbnail_style === 'style-2') { ?> 
    <div class="mr_product_thumb product_image">             
        <figure class="image-box" <?php echo esc_attr($inlineStyle); ?>>
...
<?php } elseif ($thumbnail_style === 'style-3') { ?> 
    <div class="flip-box mr_product_thumb product_image">
        <div class="flip-box-inner" <?php echo esc_attr($inlineStyle); ?>>
...
<?php } elseif ($thumbnail_style === 'style-5') { ?> 
    <div class="product_block_one mr_product_thumb product_image">
        <div class="product-inner">
            <div class="product_image" <?php echo esc_attr($inlineStyle); ?>>
                <?php   
                $images = $repeater_images;

                if ($images && isset($images['select_image_media'])) {
                    $image_urls = array();
                    $thumbnail_url = esc_url(get_the_post_thumbnail_url(get_the_ID(), 'thumbnail')); // Get the product thumbnail URL

                    foreach ($images['select_image_media'] as $key => $image_set) {
                        $image_url = isset($image_set['url']) ? esc_url($image_set['url']) : '';
                        $first_image_url = isset($images['select_image_media'][0]['url']) ? esc_url($images['select_image_media'][0]['url']) : '';
